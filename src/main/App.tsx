...
export const App: React.FC = () => {
  ...
  const [profile, setProfile] = useState<{ name: string; username: string; avatar: string | null }>({
    name: 'Kumpel', username: 'user', avatar: null,
  });

  const [balance, setBalance] = useState(0);
  const [marketData, setMarketData] = useState<{ rate: number; history: Array<{ date: string; rate: number }> }>({
    rate: 1.0, history: [],
  });
    ...
    if (payToken) {
      ...
      api.getQRInfo(payToken).then((res) => {
        if (res.success && res.amount && res.name && res.username) {
          setQrToPay({ token: payToken, amount: res.amount, name: res.name, username: res.username });
        } else {
  ...
  const sendNotification = (message: string) => {
    ...
    if (typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'granted') {
      try { new Notification('Kumpel Bank', { body: message, icon: '/logo.png' }); } catch {}
    }
  };
  ...
  const getScreenStyle = (screenName: string): React.CSSProperties => {
    ...
    return {
      ...
      visibility: Math.abs(cur - ths) <= 1 ? 'visible' : 'hidden',
    };
  };
  ...
  return (
    <div className="relative w-full h-[100dvh] overflow-hidden select-none bg-[#5491D0]">
      {successToast && (
        <div className="absolute top-5 inset-x-0 z-[60] flex justify-center px-4 animate-in fade-in slide-in-from-top-4 duration-300 pointer-events-none">
          <div className="w-full max-w-[340px] bg-white/90 dark:bg-[#1C1C1E]/90 border border-black/10 dark:border-white/10 backdrop-blur-xl rounded-[20px] p-4 shadow-xl flex items-center gap-3 pointer-events-auto">
            ...
            <span className="text-black dark:text-white text-[14px] font-semibold tracking-tight leading-tight">{successToast}</span>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 z-[70] bg-black/50 backdrop-blur-md flex items-center justify-center px-5 animate-in fade-in duration-200">
          <div className="w-full max-w-[320px] bg-white dark:bg-[#1C1C1E] border border-black/5 dark:border-white/10 rounded-[28px] p-6 flex flex-col items-center text-center shadow-2xl">
            <div className="w-14 h-14 bg-[#E33125] rounded-full flex items-center justify-center mb-3 shadow-md">
              <img src="/logo.png" className="w-7 h-7 object-contain brightness-0 invert" />
            </div>
            <h3 className="text-black dark:text-white text-[19px] font-bold tracking-tight mb-1">Оплата по QR</h3>
            ...
            <div className="w-full flex gap-2.5">
              <button onClick={() => setQrToPay(null)} className="flex-1 h-12 rounded-full bg-black/5 dark:bg-white/10 text-black dark:text-white font-semibold text-[15px] active:scale-95 transition-all">Отмена</button>
              <button onClick={handleConfirmPay} className="flex-1 h-12 rounded-full bg-[#34C759] text-white font-semibold text-[15px] shadow-md active:scale-95 transition-all">Оплатить</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="absolute inset-0 w-full h-full overflow-hidden will-change-transform z-20" style={getScreenStyle('auth_code')}>
        <AuthCodeScreen isActive={currentScreen === 'auth_code'} onBack={() => setCurrentScreen('auth')} onVerify={handleVerifyCode} />
      </div>
      ...
      <div className="absolute inset-0 w-full h-full overflow-hidden will-change-transform z-10" style={getScreenStyle('mainFlow')}>
        ...
        <div className="absolute inset-0 w-full h-full will-change-transform z-10" style={{ transform: `translateX(${activeTab === 'main' ? '0%' : '-100%'})`, transition: 'transform 450ms', visibility: activeTab === 'main' ? 'visible' : 'hidden' }}>
          <MainScreen onOpenTransfer={() => setCurrentScreen('transfer')} onOpenRequest={() => setCurrentScreen('request')} savedStyleId={savedStyleId} setSavedStyleId={setSavedStyleId} savedBgId={savedBgId} setSavedBgId={setSavedBgId} savedTheme={savedTheme} setSavedTheme={(t) => { setSavedTheme(t); setStoredTheme(t); }} setPreviewTheme={setPreviewTheme} balance={balance} transactions={transactions} isEditMode={isEditMode} setIsEditMode={setIsEditMode} profile={profile} />
        </div>
        <div className="absolute inset-0 w-full h-full will-change-transform z-10" style={{ transform: `translateX(${activeTab === 'exchange' ? '0%' : '100%'})`, transition: 'transform 450ms', visibility: activeTab === 'exchange' ? 'visible' : 'hidden' }}>
      ...
      <div className="absolute inset-0 w-full h-full overflow-hidden will-change-transform z-[40]" style={getScreenStyle('transfer')}>
        <TransferScreen isActive={currentScreen === 'transfer'} onBack={() => setCurrentScreen(activeTab)} onSuccess={(amt) => { sendNotification(`Перевод на ${amt} ₭ выполнен!`); if (token) loadData(token); }} activeStyle={activeStyle} balance={balance} currentBgImage={activeBg.image} token={token} />
      </div>
      ...
      <div className="fixed bottom-0 inset-x-0 z-[50] flex flex-col justify-end pointer-events-none transition-transform duration-500" style={{ transform: isTabBarVisible ? 'translateY(0)' : 'translateY(100%)' }}>
        <div className="absolute bottom-0 inset-x-0 h-[120px] backdrop-blur-[16px] -z-10 transition-opacity duration-500" style={{ WebkitMaskImage: 'linear-gradient(to top, black 50%, transparent 100%)', opacity: isTabBarVisible ? 1 : 0 }} />
        <div className="w-full pb-6 pt-10 pointer-events-auto"><TabBar activeTab={activeTab} onChange={(tab) => { setActiveTab(tab); setCurrentScreen(tab); }} /></div>
      </div>
    </div>
  );
};
